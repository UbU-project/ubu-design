...
**Status:** Accepted → DESIGN.md §13, §15.2.2; PLANNING_KERNEL_CONTRACT.md §6; ubu-schemas (`planning/affect-profile`, planning-response legitimization fields); ubu-planning-kernel; ubu-orchestrator; ubu-ui; ubu-devshell. Refines `UBU-D0124` (legitimization makes skeleton Plans human-viable) and builds on `UBU-D0235` (canonical timed Plan).

Affect legitimization is the Phase 1 filter that makes a skeleton Plan human-viable. Each active affect dimension — `energy` (`higher_is_better`), `stress` (`lower_is_better`), and `mood_intensity` (`lower_is_better`, arousal/volatility, not valence) — carries a sigmoid satisfaction in `[0,1]` computed from a `location`, `scale`, and `threshold` against a predicted affect observation, per `PLANNING_KERNEL_CONTRACT.md` §6. A candidate is affect-feasible only if every active dimension's satisfaction meets its threshold at the CPU-selected aggregate evaluation point; the filter records per-dimension satisfaction, an `affect_margin`, the `violated_dimensions`, and a legitimization result of `passed`, `failed`, or `needs_clarification`. The filter runs in `enforce` mode for user-facing planning and `warn_only` for onboarding, test, and stale or missing affect. The `AffectProfile` is built from the bootstrap affect-calibration answers with documented bootstrap defaults marked as temporary review priors; a missing or stale observation uses the bootstrap default with a marked warning or `warn_only`, never silently presented as current measured state. The `mood_intensity` `lower_is_better` direction is an explicit Phase 1 simplification; the `bounded_optimum` roadmap is Phase 2.

**Consequences:** `ubu-schemas` adds the `affect-profile` schema and the legitimization-result fields on the planning response, additively and under this decision; the planning kernel implements `full_legitimize` as the affect filter; the orchestrator builds the `AffectProfile` and the affect mode and surfaces the result; the UI surfaces affect legitimization on the Compact Calendar; the devshell asserts the feasible, infeasible, and stale-affect paths. This record authorizes the additive schema change; it does not add support-task insertion, semi-legitimization, value scoring, or rollout, which are later steps, and `full_legitimize` here covers the affect filter only.
